...
export const cryptoFormModal: React.FC<cryptoFormModalProps> = ({ cryptos, open, onClose, onSubmit, loading }) => {
    ...
    const { register, reset, handleSubmit, getValues, control, formState: { errors }, watch } = useForm<cryptoFormData>();
    ...
    useEffect(() => {
        if (watchcrypto) {
            const crypto = cryptos.find(({id}) => id == getValues('crypto'))
                ...
                <Box mt={4} component="form" method="post" action="#" noValidate autoComplete="off" sx={{ mt: 1 }} onSubmit={handleSubmit(onSubmit)}>
                    <Box>
                        <TextField
                            ...
                            {...register("crypto", {
                                required: "Escolha uma crypto",
                                valueAsNumber: true
                            })}
                        >
                            {cryptos?.map(({ id, code, name }, index) => (
                        ...
                        <Box mt={2}>
                            <Typography>Cotação atual: {selectedcrypto?.value.toLocaleString('pt-br',{style: 'currency', currency: 'BRL'})}R$</Typography>
                        </Box>
